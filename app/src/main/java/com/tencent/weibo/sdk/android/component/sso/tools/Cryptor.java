...
public class Cryptor {
    ...
    public static byte[] _4bytesEncryptAFrame(int i, byte[] bArr) {
        ...
        while (true) {
            s2 = (short) (i2 - 1);
            if (i2 <= 0) {
                return new byte[]{(byte) (i3 >> 8), (byte) (i3 & MotionEventCompat.ACTION_MASK), (byte) (i4 >> 8), (byte) (i4 & MotionEventCompat.ACTION_MASK)};
            }
    ...
    public byte[] encrypt(byte[] bArr, int i, int i2, byte[] bArr2) {
        ...
        for (i3 = 1; i3 <= this.pos; i3++) {
            this.plain[i3] = (byte) (rand() & MotionEventCompat.ACTION_MASK);
        }
        this.pos++;
        for (i3 = 0; i3 < 8; i3++) {
        ...
        while (this.padding <= 2) {
            if (this.pos < 8) {
                ...
                i4 = this.pos;
                this.pos = i4 + 1;
                bArr3[i4] = (byte) (rand() & MotionEventCompat.ACTION_MASK);
                this.padding++;
            }
            if (this.pos == 8) {
                encrypt8Bytes();
