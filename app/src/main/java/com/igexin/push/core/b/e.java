...
public class e implements a {
    ...
    private ContentValues a(h hVar) {
        ...
        if (hVar.b() == null || hVar.b().equals("")) {
            ...
            contentValues.put("accesstoken", com.igexin.a.a.a.a.b(hVar.b().getBytes(), g.D));
            b(hVar.b(), hVar.a());
        }
    ...
    public String a(List list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            if (list.get(i) == i.START_SERVICE) {
                str = str + FeedbackItem.STATUS_WAITING;
            } else if (list.get(i) == i.STOP_SERVICE) {
                str = str + "1";
            } else if (list.get(i) == i.IS_STARTED) {
                str = str + FeedbackItem.STATUS_SOLVED;
            } else if (list.get(i) == i.SET_SILENTTIME) {
                str = str + FeedbackItem.STATUS_RECORDED;
            }
            if (i != list.size() - 1) {
    ...
    public List c(String str) {
        ...
        for (int i = 0; i < split.length; i++) {
            if (split[i].equals(FeedbackItem.STATUS_WAITING)) {
                arrayList.add(i.START_SERVICE);
            } else if (split[i].equals("1")) {
                arrayList.add(i.STOP_SERVICE);
            } else if (split[i].equals(FeedbackItem.STATUS_SOLVED)) {
                arrayList.add(i.IS_STARTED);
            } else if (split[i].equals(FeedbackItem.STATUS_RECORDED)) {
