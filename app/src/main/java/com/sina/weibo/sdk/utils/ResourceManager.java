...
public class ResourceManager {
    ...
    private static String getAppropriatePathOfDrawable(Context context, String str) {
        ...
        String stringBuilder = new StringBuilder(String.valueOf(currentDpiFolder)).append("/").append(str).toString();
        LogUtil.i(TAG, "Maybe the appropriate path: " + stringBuilder);
        if (isFileExisted(context, stringBuilder)) {
            return stringBuilder;
        }
        LogUtil.d(TAG, "Not the correct path, we need to find one...");
        for (int i = 0; i < PRE_INSTALL_DRAWBLE_PATHS.length; i++) {
            if (obj != null) {
                String stringBuilder2 = new StringBuilder(String.valueOf(PRE_INSTALL_DRAWBLE_PATHS[i])).append("/").append(str).toString();
                if (isFileExisted(context, stringBuilder2)) {
                    return stringBuilder2;
                }
            } else if (currentDpiFolder.equals(PRE_INSTALL_DRAWBLE_PATHS[i])) {
                obj = 1;
    ...
    private static String getCurrentDpiFolder(Context context) {
        ...
        if (i <= 240 || i > TTFMSongEntity.BITRATE_DEFAULT) {
