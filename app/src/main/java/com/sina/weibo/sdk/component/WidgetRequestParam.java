...
public class WidgetRequestParam extends BrowserRequestParamBase {
    ...
    private String buildUrl(String str) {
        ...
        stringBuilder.append("?").append("version").append("=").append(WBConstants.WEIBO_SDK_VERSION_CODE);
        if (!TextUtils.isEmpty(this.mAppKey)) {
            stringBuilder.append("&").append("source").append("=").append(this.mAppKey);
        }
        if (!TextUtils.isEmpty(this.mToken)) {
            stringBuilder.append("&").append("access_token").append("=").append(this.mToken);
        }
        Object aid = Utility.getAid(this.mContext, this.mAppKey);
        if (!TextUtils.isEmpty(aid)) {
            stringBuilder.append("&").append("aid").append("=").append(aid);
        }
        if (!TextUtils.isEmpty(this.mAppPackage)) {
            stringBuilder.append("&").append("packagename").append("=").append(this.mAppPackage);
        }
        if (!TextUtils.isEmpty(this.mHashKey)) {
            stringBuilder.append("&").append("key_hash").append("=").append(this.mHashKey);
        }
        if (!TextUtils.isEmpty(this.mAttentionFuid)) {
            stringBuilder.append("&").append(REQ_PARAM_ATTENTION_FUID).append("=").append(this.mAttentionFuid);
        }
        if (!TextUtils.isEmpty(this.mCommentTopic)) {
            stringBuilder.append("&").append(REQ_PARAM_COMMENT_TOPIC).append("=").append(this.mCommentTopic);
        }
        if (!TextUtils.isEmpty(this.mCommentContent)) {
            stringBuilder.append("&").append("content").append("=").append(this.mCommentContent);
        }
        if (!TextUtils.isEmpty(this.mCommentCategory)) {
            stringBuilder.append("&").append(REQ_PARAM_COMMENT_CATEGORY).append("=").append(this.mCommentCategory);
        }
