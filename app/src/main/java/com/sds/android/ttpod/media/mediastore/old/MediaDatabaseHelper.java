...
import com.sds.android.ttpod.media.mediastore.old.MediaStore.ArtistColumns;
...
public final class MediaDatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onUpgrade(SQLiteDatabase sQLiteDatabase, int i, int i2) {
        g.d(LOG_TAG, "databaseUpgrade:" + i + "->" + i2);
        if (i <= 24) {
            try {
                upgradeToVer25(sQLiteDatabase);
            } catch (Throwable e) {
                g.b(LOG_TAG, "database upgrade to 25 error", e);
            }
        }
        if (i < 26) {
            try {
                ...
                g.b(LOG_TAG, "database upgrade to 26 error", e2);
            }
        }
        if (i < 27) {
            try {
                ...
                g.b(LOG_TAG, "database upgrade to 27 error", e22);
            }
        }
        if (i < 28) {
            try {
                ...
                g.b(LOG_TAG, "database upgrade to 28 error", e222);
            }
        }
        if (i < 29) {
            try {
                ...
                g.b(LOG_TAG, "database upgrade to 29 error", e2222);
            }
        }
        if (i < 30) {
            try {
                ...
                g.b(LOG_TAG, "database upgrade to 30 error", e22222);
            }
        }
        if (i < 31) {
            try {
                ...
                g.b(LOG_TAG, "database upgrade to 31 error", e222222);
            }
        }
        if (i < 32) {
            try {
                ...
                g.b(LOG_TAG, "database upgrade to 32 error", e2222222);
            }
        }
        if (i < 33) {
            try {
        ...
        if (i < 35) {
            try {
                ...
                g.b(LOG_TAG, "database upgrade to 35 error", e22222222);
            }
        }
        if (i < DATABASE_VERSION_38) {
            try {
                ...
                g.b(LOG_TAG, "database upgrade to 38 error", e222222222);
            }
        }
        if (i < DATABASE_VERSION_41) {
            try {
                ...
                g.b(LOG_TAG, "database upgrade to 41 error", e2222222222);
            }
        }
        if (i < DATABASE_VERSION_45) {
            try {
                ...
                g.b(LOG_TAG, "database upgrade to 45 error", e22222222222);
            }
        }
        if (i < 46) {
            try {
                ...
                g.b(LOG_TAG, "database upgrade to 46 error", e222222222222);
            }
        }
    }
    ...
    private void upgradeToVer25(SQLiteDatabase sQLiteDatabase) {
        addColumn(sQLiteDatabase, DATABASE_TABLE_MEDIAS, MediasColumns.COMMENT, "TEXT");
    }
    ...
    private void upgradeToVer27(SQLiteDatabase sQLiteDatabase) {
        processMetaDigitalKey(sQLiteDatabase, DATABASE_TABLE_MEDIAS, MediasColumns.TITLE_DIGITAL_KEY, true);
        processMetaDigitalKey(sQLiteDatabase, "artists", ArtistColumns.ARTIST_DIGITAL_KEY, true);
    }

    private void upgradeToVer28(SQLiteDatabase sQLiteDatabase) {
        addColumn(sQLiteDatabase, DATABASE_TABLE_MEDIAS, MediasColumns.SONG_ID, "INTEGER");
    }
    ...
    private void upgradeToVer38(SQLiteDatabase sQLiteDatabase) {
        addColumn(sQLiteDatabase, DATABASE_TABLE_ONLINE_MEDIAS, MediasColumns.ARTIST_ID, "INTEGER");
    }
        ...
        synchronized (this) {
            ...
            try {
                ...
                try {
                    ...
                    g.a(LOG_TAG, String.format("lookdbversion: curDbVersion %d , support Version_Newest %d.", new Object[]{Integer.valueOf(version), Integer.valueOf(46)}));
                    if (version > 46) {
                        try {
                            openDatabase.close();
                            g.a(LOG_TAG, String.format("curVersion %d > Version_Newest %d will delete db.", new Object[]{Integer.valueOf(version), Integer.valueOf(46)}));
                            e.h(path);
                        } catch (Exception e) {
                            g.c(LOG_TAG, "TTMessage1:" + e.getMessage());
                        }
                    } else {
                    ...
                    if (sQLiteDatabase2 != null) {
                        try {
                            ...
                            g.e(LOG_TAG, "TTMessage2:" + e2.getMessage());
                        }
                    }
                } catch (Throwable th2) {
                    th = th2;
